...
class CustomInfiniteGroupedList<ItemType, GroupBy, GroupTitle>
    extends StatefulWidget {
  ReportFilterChips filterChips;

  factory CustomInfiniteGroupedList({
    ...
    required ReportFilterChips filterChips,
  }) {
    return CustomInfiniteGroupedList._(
  ...
  factory CustomInfiniteGroupedList.gridView({
    required Widget Function(ItemType item) itemBuilder,
    required GroupBy Function(ItemType item) groupBy,
    required Widget Function(
      GroupTitle title,
...
class _InfiniteGroupState<ItemType, GroupBy, GroupTitle>
    extends State<CustomInfiniteGroupedList<ItemType, GroupBy, GroupTitle>> {
  bool loading = true;
  bool hasError = false;
  dynamic error;
        ...
        : RefreshIndicator(
            ...
            child: groupTitles.isEmpty
                ...
                : CustomScrollView(
                    ...
                    physics: widget.physics,
                    slivers: <Widget>[widget.filterChips]
                        .followedBy(groupTitles.map<Widget>((title) {
  ...
  Map<GroupTitle, List<ItemType>> groupItems(List<ItemType> items) {
    ...
    if (widget.sortGroupBy != null) {
      groupedItems.forEach((key, value) {
        value.sort((a, b) {
          return (widget.sortGroupBy!(b) as DateTime).compareTo(widget.sortGroupBy!(a) as DateTime);
        });
      });
    }
  ...
  void sortGroupTitles() {
    var vandaag = groupTitles.where((element) => element == "Vandaag").firstOrNull;
    var gisteren = groupTitles.where((element) => element == "Gisteren").firstOrNull;

    groupTitles.removeWhere((element) => element == "Vandaag");
    groupTitles.removeWhere((element) => element == "Gisteren");

    if(gisteren != null){
      groupTitles.insert(0, gisteren);
    }
    if(vandaag != null){
      groupTitles.insert(0, vandaag);
    }

  }
}

/// This is the controller for the [CustomInfiniteGroupedList].
///
